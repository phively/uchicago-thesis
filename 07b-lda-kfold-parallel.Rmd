...
perps_new <- data.frame(
  model = c(38, 39, 41, 42) %>% rep(each = K),
  fold = rep(1:K, times = 4),
  # Add time elapsed in hours
  time = c(t38, t39, t41, t42) %>% as.numeric(units = "hours") %>% rep(each = K)
)
...
perps_new <- perps_new %>% mutate(out_sample = c(perp38, perp39, perp41, perp42))
...
```{r}
perplexity %>%
  ggplot(aes(x = model)) + geom_point(aes(y = in_sample, color = "In-sample")) + geom_line(aes(y = in_sample, color = "In-sample")) +
  geom_point(aes(y = out_sample, color = "Out-sample")) + geom_line(aes(y = out_sample, color = "Out-sample")) +
  labs(x = "topics", y = "perplexity", color = "", title = paste(K, "-fold cross-validation perplexity", sep = ""))
```

The minimum is at $K = 40$.
...
gc()
now <- Sys.time()
models40f <- LDA(dtm = sparse, index = rand_ind, ntopics = 40, seed = 19275, keep = 1)
(t40f <- Sys.time() - now)
save(models40f, file = "results/lda_finl_40.zip", compress = "xz", compression_level = 1)
```