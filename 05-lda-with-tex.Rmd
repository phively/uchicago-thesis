---
title: "LDA with TeX code intact"
output: html_notebook
---

# Goals

  * Repeat the previous exercise but leaving the LaTeX code in the corpus
...
Source code: [read-PostsData-zip-with-tex.R](https://github.com/phively/uchicago-thesis/blob/master/R/read-PostsData-zip-with-tex.R)

```{r, message=F}
source("R/read-PostsData-zip-with-tex.R")
```
...
# Number of additional stopwords
...
```{r}
# Save sparse term matrix
save(sparse, file = "data/sparse_matrix_w_tex.zip", compress = "xz")
```
```{r}
topic <- LDA(sparse, k = 15, method = "VEM", control = list(seed = 48029, verbose = 1))
save(topic, file = "results/lda15-with-tex.zip", compress = "xz")
```
