...
xval <- function(dtm, index, ntopics, seed) {
  ...
  for (i in 1:K) {
    ...
    models[[i]] <- LDA(this_data, k = ntopics, method = "VEM", control = list(seed = seed, verbose = 1))
  }
...
```{r}
now <- Sys.time()
models5 <- xval(dtm = sparse, index = rand_ind, ntopics = 5, seed = 74749)
...
```{r}
now <- Sys.time()
models10 <- xval(dtm = sparse, index = rand_ind, ntopics = 10, seed = 90672)
...
```{r}
now <- Sys.time()
models15 <- xval(dtm = sparse, index = rand_ind, ntopics = 15, seed = 53177)
...
```{r}
now <- Sys.time()
models20 <- xval(dtm = sparse, index = rand_ind, ntopics = 20, seed = 45281)
...
```{r}
now <- Sys.time()
models25 <- xval(dtm = sparse, index = rand_ind, ntopics = 25, seed = 86244)
...
```{r}
now <- Sys.time()
models30 <- xval(dtm = sparse, index = rand_ind, ntopics = 30, seed = 41531)
