...
# Parallel run
gc()
nc_5_20 <- foreach(k = seq(5, 20, by = 5)) %dopar% {
...
save(nc_5_20, file = "results/nc_5_20.zip", compress = "xz", compression_level = 1)
```
...
# Parallel run
gc()
nc_25_40 <- foreach(k = seq(25, 40, by = 5)) %dopar% {
  # Cross-validated sets
  foreach(i = 1:5) %dopar% {
    stm(holdout[[i]]$documents, holdout[[i]]$vocab, K = k, init.type = "Spectral", ngroups = 5,
      control = list(maxV = length(holdout[[i]]$vocab)), sigma.prior = 1)
...
save(nc_25_40, file = "results/nc_25_40.zip", compress = "xz", compression_level = 1)
```
...
# Parallel run
gc()
nc_45_60 <- foreach(k = seq(45, 60, by = 5)) %dopar% {
  # Cross-validated sets
  foreach(i = 1:5) %dopar% {
    stm(holdout[[i]]$documents, holdout[[i]]$vocab, K = k, init.type = "Spectral", ngroups = 5,
      control = list(maxV = length(holdout[[i]]$vocab)), sigma.prior = 1)
...
save(nc_45_60, file = "results/nc_45_60.zip", compress = "xz", compression_level = 1)
```
