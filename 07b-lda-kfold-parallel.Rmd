...
perplexity <- data.frame(
  model = c(3, 5, 10, 15, 20, 25, 30) %>% rep(each = K),
  fold = rep(1:K, times = 7),
  # Add time elapsed in hours
  time = c(t3, t5, t10, t15, t20, t25, t30) %>% as.numeric(units = "hours") %>% rep(each = K)
)
...
perplexity <- perplexity %>% mutate(
  out_sample = c(perp3, perp5, perp10, perp15, perp20, perp25, perp30) %>% unlist()
)
