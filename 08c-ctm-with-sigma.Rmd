...
# 750
ctm_750 <- stm(holdout1$documents, holdout1$vocab, K = 750, init.type = "Spectral", ngroups = 5,
      control = list(maxV = length(holdout1$vocab)))
save(ctm_750, file = "results/ctm_750.zip", compress = "xz", compression_level = 1)

# 1000
ctm_1000 <- stm(holdout1$documents, holdout1$vocab, K = 1000, init.type = "Spectral", ngroups = 5,
      control = list(maxV = length(holdout1$vocab)))
save(ctm_1000, file = "results/ctm_1000.zip", compress = "xz", compression_level = 1)
```
...
ctm_heldout_lik <- test %>% data.frame(
  K = rep(seq(5, 25, by = 5), each = n) %>%
    c(rep(seq(30, 100, by = 10), each = n), rep(c(125, 150, 175, 200, 250), each = n)),
  lik = .
)
```

```{r}
# Additional heldout samples
